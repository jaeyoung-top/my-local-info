...
export default function NewsCard({ item, color }: { item: InfoItem; color: 'indigo' | 'orange' }) {
  ...
  const className = `news-card block overflow-hidden group transition-all duration-300 ${color === 'orange' ? 'border-2 border-[#F25C05] shadow-[0_5px_15px_rgba(242,92,5,0.15)] rounded-2xl cursor-pointer hover:shadow-xl' : 'border border-gray-100 rounded-xl cursor-pointer hover:border-[#1D428A] hover:shadow-lg'}`;
  ...
  return (
    <Link href={`/blog/${item.id}`} className={className}>
      {inner}
    </Link>
  );
}
